...
{
    public class ExpensivePaymentService : PaymentServiceProvider
    {
        public override bool FulfillPayment(CreditCard creditCard, int retryAttempts)
        {
            if (CheckServiceAvailability())
            {
                ...
                {
                    string failureReason = "System error while wiring monney";
        ...
        private bool WireMonney(CreditCard creditCard)
        {
            try
            {
                //  <payment logic for ExpensivePaymentService>
            }
